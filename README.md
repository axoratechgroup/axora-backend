...
una para el remitente y otra para el destinatario. Cada correo se entrega de forma
individual a través de la Vercel Function existente y AWS SES.

- `src/services/transactionEmail.ts`: plantilla HTML con estilos inline, logo oficial,
  texto plano, importes persistidos, contraparte/motivo y tasa/comisión para cambios.
  Escapa todo texto dinámico al construir HTML y muestra fechas en UTC.
- `src/services/notificationOutbox.ts`: registra los destinatarios en la misma
  transacción SQL del movimiento. Después del `COMMIT`, reclama cada fila mediante
  un `UPDATE` condicional y registra `PROCESSING`, `SENT` o `FAILED`.
- `src/services/email.ts`: transporte autenticado a Vercel, con timeout de 10 segundos
  y redirecciones deshabilitadas. AWS se configura exclusivamente en Vercel.
...
5. Comprobar un correo para una carga/cambio y dos correos personalizados para una
   transferencia. Consultar el outbox para verificar cada resultado:

```sql
SELECT transaction_id, recipient_role, status, attempts, last_error, sent_at
FROM notification_outbox ORDER BY created_at DESC LIMIT 20;
```

### Fallos y reintentos

Si falla la inserción del outbox, el movimiento completo hace `ROLLBACK`. Si falla el
envío externo después de `COMMIT`, el movimiento sigue confirmado y el correo queda
...
no se expone un endpoint público de reenvío. Si el proceso se interrumpe o falla el
registro de éxito, revisar filas `PROCESSING` antes de cualquier reenvío. Un timeout
puede ocurrir después de que SES haya aceptado el correo: no se garantiza entrega
exactamente una vez y un reintento puede duplicarlo.

Las pruebas mockean PostgreSQL y el transporte: verifican plantillas, escape HTML,
envíos separados, estados, exclusión de filas reclamadas y orden enqueue/COMMIT/envío.
La migración y la recepción real requieren verificación en un entorno autorizado.
